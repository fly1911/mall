	...
	<body>
		...
		<div class="container">
							...
							<c:if test="${not empty cartList}">
								<c:forEach items="${cartList}" var="item">
									<tr class="active">
										...
										<td width="30%">
											<a href="${pageContext.request.contextPath}/productServlet?method=get&pid=${item.pid}">${item.pname}</a>
										</td>
										<td width="20%" id="price_${item.id}">
											${item.price}
										</td>
										<td width="10%">
											<input type="text" name="quantity" onchange="updateCart('${item.id}', this.value)" value="${item.count}" maxlength="4" size="10">
										</td>
										<td width="15%">
											<span class="subtotal" id="subtotal_${item.id}">${item.subtotal}</span>
										</td>
										<td>
											<a href="javascript:delCartItem('${item.id}');" class="delete">删除</a>
										</td>
									</tr>
								</c:forEach>
							</c:if>
			...
			<div style="margin-right:130px;">
				...
				登录后确认是否享有优惠&nbsp;&nbsp;
			</em> 赠送积分: <em style="color:#ff6600;">0</em>&nbsp; 商品金额: <strong style="color:#ff6600;" id="totalPrice">￥${totalPrice}元</strong>
				</div>
...
<script type="text/javascript">
	function delCartItem(id) {
		$.ajax({
			...
			method:"POST",
			data:"method=delCartItem&id="+id,
			dataType:"text",
			success:function(data){
				if (data == "1") {
					location.href = "${pageContext.request.contextPath}/cartServlet?method=list";
				}
			}
		});
	}
	...
	function updateCart(id, count) {
		...
		for (var i = 0;i < stArr.length;i++) {
			var st = parseInt($("#"+stArr[i].id).text());
			total = total+st;
		}
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/cartServlet",
			method:"POST",
			data:"method=updateCart&id="+id+"&count="+count,
			dataType:"text",
			success:function(data){
				if (data == "1") {
					location.href = "${pageContext.request.contextPath}/cartServlet?method=list";
